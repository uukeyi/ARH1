...
import { ILandingBlockElement, landingElPrototype } from '../../interfaces/landingPageResponse';
...
const ServicePage: React.FC = () => {
  ...
  const slidesGallery = photosGallery.map(({ src, key, width, height, images }) => ({
    src,
    key,
    width,
    height,
          ...
          {mainContent?.map((element: ILandingBlockElement, index): any => {
            if (element.typeId === 1) {
              ...
              return (
                <p
                  key={index}
                  className={element.aosAnimation === 'liElement' ? listItem : ''}
                  onClick={() => {
                    if (isAuthSettings.isAdmin) {
                      setIsOpen(true);
                      setElSettings(element);
