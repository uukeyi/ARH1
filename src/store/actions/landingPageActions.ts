...
interface ICreateBlock {
   ...
   isDeleted?: boolean;
   elements?: [];
   createWithInitialData?: boolean;
   initialElements?: IInitialElement[];
   setError: (value: boolean | ((prevVar: boolean) => boolean)) => void;
}
interface IInitialElement {
   value: string;
   orderIndex: number | string;
   aosAnimation: string;
...
export const getLandingPageBlock = createAsyncThunk<
   ...
   { rejectValue?: string }
>(
   `landingPageSlice/getLandingPageBlock`,
...
>("landingPageSlice/createBlock", async (params, { rejectWithValue }) => {
   try {
      ...
      if (params.createWithInitialData) {

         await params.initialElements?.map((el: any) => {
            $host<ILandingBlockElement>({
               method: "POST",
               url: "/api/LandingBlocks/elements",
               data: {
...
export const deleteEl = createAsyncThunk<
   ...
   IDeleteEl,
   { rejectValue?: string }
>("landingPageSlice/deleteEl", async (params, { rejectWithValue }) => {
   try {
