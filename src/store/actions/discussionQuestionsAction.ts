...
export const getDiscussionQuestions = createAsyncThunk<IDiscussionGetResponse, (value: boolean | ((prevVar: boolean) => boolean)) => void , { rejectValue?: string }>(
  'discussionQuestionsSlice/getDiscussionQuestions',
  async (setError, { rejectWithValue }) => {
    try {
      const response = await axios.get<IDiscussionGetResponse>('http://194.87.238.163/api/Discussions');
...
>('discussionQuestionsSlice/getDiscussionDetails', async (config, { rejectWithValue }) => {
  try {
    const response = await axios.get<IDiscussion>(`http://194.87.238.163/api/Discussions/${config.id}`);
    return response.data;
  } catch (error: any) {
